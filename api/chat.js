...
// 1) If there are no new messages since this session loaded the chat, return; 2) set the user contact `hasUnread` to false; 3) return chat
module.exports = async (req, res) => {
	...
	if (isNotFirstLoad && !contact.hasUnread)
		return res.status(204).send('No new messages'); // 1
	...
	res.json({ users, pastMessages: chat.pastMessages }); // 3
};