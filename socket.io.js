...
const getChat = (_id, name, contacts) => async to => {
	...
	socket.emit('get chat', {
		to, pastMessages: chat.pastMessages,
		contactIsChatUserNumber: contacts[i].chatUserNumber
	});
}

const postMessage = (from, contacts, _id, socket) => async ({ to, text }) => {
	const when = new Date();
	...
	if (!!io.sockets.adapter.rooms[to].sockets) {
		io.to[to].emit('get message', msgObj);
...
io.on('connection', socket => {
	...
	socket.on('get chat', getChat(_id, name, contacts));
	socket.on('post message', postMessage(name, contacts, _id, socket));
});
